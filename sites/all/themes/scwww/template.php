...
// Add Zen Tabs styles
if (theme_get_setting('scwww_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'scwww') .'/css/tabs.css');
}

function scwww_preprocess_html(&$vars) {
  global $user, $language;

  // Add role name classes (to allow css based show for admin/hidden from user)
  foreach ($user->roles as $role){
    $vars['classes_array'][] = 'role-' . scwww_id_safe($role);
  }
  ...
  if (!$vars['is_front']) {
    ...
    $vars['classes_array'][] = scwww_id_safe('page-'. $path);
    $vars['classes_array'][] = scwww_id_safe('section-'. $section);
  ...
  // Add IE classes.
  if (theme_get_setting('scwww_ie_enabled')) {
    $scwww_ie_enabled_versions = theme_get_setting('scwww_ie_enabled_versions');
    if (in_array('ie8', $scwww_ie_enabled_versions, TRUE)) {
...
function scwww_preprocess_page(&$vars, $hook) {
...
function scwww_preprocess_node(&$vars) {
  ...
  // Merge first/last class (from scwww_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
  ...
  // Special treatment for question--list-sidebar view mode
  if ($vars['view_mode'] == "list_sidebar") {
    $hashtags = array();
    foreach (field_get_items('node', $vars['node'], 'field_hashtags') as $field) {
      array_push($hashtags, $field['title']);
    }
    $vars['hashtags_linked'] = array(
      '#theme' => 'link',
      '#text' => join(' ',$hashtags),
      '#path' => 'node/' . $vars['nid'],
      '#options' => array(
...
/* function scwww_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function scwww_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('scwww_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('scwww_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('scwww_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('scwww_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('scwww_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
...
function scwww_id_safe($string) {
    // Replace with dashes anything that isn't A-Z, numbers, dashes, or underscores - updated for Drupal 7
 ...
 *
 */
function scwww_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = scwww_id_safe($element['#title']);
 ...
 */
function scwww_preprocess_menu_local_task(&$variables) {
 ...
 */
function scwww_menu_local_tasks(&$variables) {
  $output = '';
...
/**
 * Remove system stylesheets from Front end theme
 */
/* function scwww_css_alter(&$css) {
 ...
 */
function scwww_preprocess_block(&$vars, $hook) { //change mytheme to your theme name
    $block = $vars['block'];
    ...
    if (!empty($block->subject)) {
        ...
        // $cssid = 'block-' . scwww_id_safe($block->title);
        $cssid = scwww_id_safe($block->title);
    }
    else  {
        // If no "Block title", create css id attribute the traditional way.
        $cssid = "$block->module-$block->delta";
        //$cssid ="block-$block->delta";
    }
    $vars['block_html_id'] = $cssid;
}

/**
 * Check for external url and set target attribute to open in new tab/window
 */
function scwww_link($variables) {
